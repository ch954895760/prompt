  ...
  <div>
    ...
    >
      ...
      <button v-if="hasChildren()" @click.stop="emit('toggle', category.id)"
        class="w-5 h-5 rounded-md flex items-center justify-center transition-colors hover:bg-[var(--bg-tertiary)] flex-shrink-0"
      >
      ...
      <div class="hidden group-hover:flex items-center gap-1">
        <button @click.stop="emit('addChild', category.id)"
          class="p-1 rounded hover:bg-[var(--bg-tertiary)]" style="color: var(--text-muted);"
        >
          <Plus class="w-3 h-3" />
        </button>
