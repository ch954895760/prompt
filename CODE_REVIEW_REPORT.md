...
| 功能点 | 状态 | 说明 |
|--------|------|------|
| 主题切换 | ✅ 已实现 | 前端localStorage持久化，支持暗黑/明亮模式 |
...
| P2 | 安全问题 | 部分API端点缺少@PreAuthorize权限校验注解 | 安全审计 | 为敏感端点添加注解完善权限控制 |

---

## 5. 已完成功能 ✓

### 分类拖拽排序（2026-04-27 完成）

**实现内容：**
- **后端API**: 新增 `POST /categories/sort` 接口，支持批量更新分类排序和层级关系
- **前端交互**: 
  - 添加「排序模式」切换按钮
  - 支持拖拽调整分类顺序（同层级排序）
  - 支持拖拽变更层级关系（成为子分类）
...
**关键文件变更：**
- `CategoryController.java` - 新增排序接口
- `CategoryService.java` - 添加 `updateSortOrder` 方法
- `CategoryCreateRequest.java` - 添加排序请求DTO
- `CategoryManage.vue` - 集成拖拽排序功能
- `CategoryNode.vue` - 添加拖拽事件支持
- `category.ts` - 添加排序API调用
...
1. **【低】优化数据导入导出** - 支持标签的完整导入导出，提升数据迁移体验
2. **【低】API权限加固** - 为敏感端点添加@PreAuthorize注解，完善安全审计
